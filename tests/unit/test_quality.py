...
class TestSilentFailureWeights:
    """静默失败场景专用权重（PRD §12.2 场景 D 优化）。"""

    def test_silent_failure_weights_sum_to_one(self):
        from app.quality.scorer import _SILENT_FAILURE_DIMENSION_WEIGHTS

        total = sum(_SILENT_FAILURE_DIMENSION_WEIGHTS.values())
        assert abs(total - 1.0) < 0.001

    def test_silent_failure_weights_cover_all_dimensions(self):
        from app.quality.scorer import _SILENT_FAILURE_DIMENSION_WEIGHTS

        for d in ContextDimension:
            assert d in _SILENT_FAILURE_DIMENSION_WEIGHTS, f"维度 {d} 缺少权重"

    def test_detect_silent_failure_with_spec_diffs(self):
        """无异常堆栈 + spec_diffs 非空 → 判定为静默失败。"""
        from app.quality.scorer import _is_silent_failure_scenario

        dc = _dc(exception=None, spec_diffs=[{"field": "a", "expected": "x", "actual": None}])
        assert _is_silent_failure_scenario(dc) is True

    def test_detect_silent_failure_with_network(self):
        """无异常堆栈 + network_trace 非空 → 判定为静默失败。"""
        from app.quality.scorer import _is_silent_failure_scenario

        dc = _dc(exception=None, network_trace=[{"url": "/api/order", "status": 200}])
        assert _is_silent_failure_scenario(dc) is True

    def test_not_silent_failure_with_exception_stack(self):
        """有异常堆栈 → 非静默失败（即使有 spec_diffs）。"""
        from app.quality.scorer import _is_silent_failure_scenario

        dc = _dc(spec_diffs=[{"field": "a", "expected": "x", "actual": None}])
        assert _is_silent_failure_scenario(dc) is False

    def test_not_silent_failure_with_empty_context(self):
        """无异常堆栈且无替代信号 → 非静默失败（避免虚高）。"""
        from app.quality.scorer import _is_silent_failure_scenario

        dc = _dc(exception=None, spec_diffs=None, network_trace=None)
        assert _is_silent_failure_scenario(dc) is False

    def test_silent_failure_uses_dedicated_weights(self):
        """静默失败场景应使用专用权重（TRACE 降权、SPEC/NETWORK 提权）。"""
        from app.quality.scorer import (
            _DIMENSION_WEIGHTS,
            _SILENT_FAILURE_DIMENSION_WEIGHTS,
        )

        assert _SILENT_FAILURE_DIMENSION_WEIGHTS[ContextDimension.SPEC] > _DIMENSION_WEIGHTS[
            ContextDimension.SPEC
        ]
        assert _SILENT_FAILURE_DIMENSION_WEIGHTS[ContextDimension.NETWORK] > _DIMENSION_WEIGHTS[
            ContextDimension.NETWORK
        ]
        assert _SILENT_FAILURE_DIMENSION_WEIGHTS[ContextDimension.TRACE] < _DIMENSION_WEIGHTS[
            ContextDimension.TRACE
        ]

    def test_silent_failure_scenario_scores_higher_than_default(self):
        """同一静默失败上下文，专用权重下完整度应高于默认权重。"""
        ...
        ctx = _agent_ctx(
            debug_overrides={
                ...
                "git_blame": None,
                "recent_diffs": None,
                "network_trace": [{"url": "POST /api/order", "status": 200}],
                "ui_events": None,
                "spec_diffs": [
                    {"field": "body.order_id", "expected": "非空字符串", "actual": None},
                ],
                "related_specs": [{"file": "specs/order_api.md", "content": "order_id 必填"}],
                "runtime": {"pid": 1, "cpu_percent": 8.0, "memory_mb": 200.0, "thread_count": 6},
            },
            repair_overrides={
                "prior_analysis": {
                    "root_cause": "order_id 未回填",
                    "impact": "前端无法获取订单号",
                    "fix": "插入后回填 ID",
                    "confidence": "medium",
                    "analysis_source": "llm",
                    "case_confidence": 0.90,
                    "verify_count": 2,
                },
                "fault_locations": [
                    {
        ...
        report = evaluate(ctx)
        # 静默失败场景应达到 PRD 预期（≥0.61）
        assert report.overall_score >= 0.61
