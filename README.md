...
>
> ⚠️ **多 Worker 部署限制**：Dashboard 实时 SSE 推送（`/api/dashboard/stream`）和 MCP SSE 长连接均基于进程内内存 `asyncio.Queue` 实现。多 Gunicorn Worker 部署时，每个 Worker 独立维护自己的事件总线，跨 Worker 事件不互通，SSE 推送仅对连接所在 Worker 的客户端生效。**单 Worker 部署无此限制**。如需多 Worker 场景下的 SSE 互通，建议使用 Redis Pub/Sub 或外部消息队列扩展。
...
| 测试基线 | 单元 `672 passed / 6 skipped / 0 failed`（含 AI Debug Agent Phase 1 63 项 + Phase 2 53 项 + Dashboard SSE 18 项 + Qdrant 适配器 23 项 + L3 预热 12 项 + 三轨并行 104 项 + 安全 Agent 严重等级 7 项 + SSEHub 线程安全 7 项 + Dashboard 事件总线 8 项） |
