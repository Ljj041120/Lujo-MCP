    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "堆栈每一帧对应的源码片段（自动定位，无需再单独读取文件）。"
                "推荐宿主AI拿到这份数据后自行分析根因，不需要额外调用 analyze_with_llm。"
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "trace_id": {"type": "string", "description": "可选，不传则取最新一条追踪记录"}
                },
            },
        ),
        Tool(
        ...
        Tool(
            name="ingest_error",
            description=(
                "供任意语言/进程主动上报一条错误（不限于Python）。"
                "字段与get_stacktrace返回结构一致，上报后会自动去重、脱敏并进入统一调试上下文。"
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "frames": {
                        "type": "array",
                        "description": "堆栈帧列表，每项包含 file/line/function/code_context",
                        "items": {"type": "object"},
                    },
